...
def generate_script():
    ...
    prompt = """请为《围城》（作者：钱钟书）生成一段6000-8000字的书籍讲解稿。
...
def main():
    ...
    # Step 1: 生成完整讲解稿（不限制字数）
    logger.info("\n--- Step 1: 生成完整讲解稿 ---")
    script = generate_script()
    script_file = OUTPUT_DIR / "data" / f"{title}_full_script.txt"
    ...
    # Step 3: 生成男声音频
    logger.info("\n--- Step 3: 生成男声音频 ---")
    audio_path = OUTPUT_DIR / "audios" / f"{title}_male_full.mp3"
    gen_audio_male(script, audio_path)

    # Step 4: 生成封面
    logger.info("\n--- Step 4: 生成封面 ---")
    cover_path = OUTPUT_DIR / "covers" / f"{title}_full.jpg"
    generate_cover(title, BOOK_INFO["author"], cover_path, BOOK_INFO["coverColor"])

    # Step 5: 上传（确保用最新token）
    logger.info("\n--- Step 5: 上传云存储 ---")
    access_token = get_access_token()
    audio_file_id = upload_file(access_token, env_id, str(audio_path), f"audios/{title}_male_full.mp3") if audio_path.exists() else None
    cover_file_id = upload_file(access_token, env_id, str(cover_path), f"covers/{title}_full.jpg") if cover_path and cover_path.exists() else None
    ...
    book_id = update_book(access_token, env_id, "c9a02d0d6a141ac40097f6506c4bc2fb", data_obj)
